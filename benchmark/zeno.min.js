...
class p extends EventTarget {
  constructor(e) {
    ...
    let t;
    u.includes("worker") || u.endsWith(".ts") ? t = u : o ? t = "zeno_worker.js" : t = import.meta.url.replace(/zeno(\.min)?\.js(\?.*)?$/, "zeno_worker.js"), console.log("Zeno Worker Source:", t);
    const n = `import "${t}";`, i = new Blob([n], { type: "application/javascript" }), s = URL.createObjectURL(i);
  ...
  async solveInWorker(e) {
    return new Promise((o, t) => {
      const n = (r) => {
        ...
        a === "SOLVED" ? (this.worker.removeEventListener("message", n), this.worker.removeEventListener("error", i), c.memoryUsage = r.data.memory, o(c)) : a === "ERROR" && (this.worker.removeEventListener("message", n), this.worker.removeEventListener("error", i), t(new Error(b)));
      }, i = (r) => {
        this.worker.removeEventListener("message", n), this.worker.removeEventListener("error", i), t(new Error(`Worker Error: ${r.message}`));
      };
      this.worker.addEventListener("message", n), this.worker.addEventListener("error", i);
      let s = "";
      if (window.ZENO_WASM_URL)
        s = window.ZENO_WASM_URL;
      else if (import.meta.url) {
        if (s = import.meta.url.replace(/zeno(\.min)?\.js(\?.*)?$/, "zeno.wasm"), s === import.meta.url)
          try {
            s = new URL("zeno.wasm", self.location).href;
          } catch {
            try {
              s = new URL("/zeno.wasm", window.location.href).href;
            } catch {
              s = "zeno.wasm";
            }
          }
...
class g extends HTMLElement {
  constructor() {
    ...
    this.shadow = this.attachShadow({ mode: "open" }), this.shadow.innerHTML = m;
  }
  ...
  showWasmBanner() {
    var _a;
    this.wasmBanner && (this.wasmBanner.classList.add("visible"), this.wasmBanner.innerText = this.getAttribute("zeno-i18n-wasm-banner") || "Enable WASM for significantly faster solving", (_a = this.container) == null ? void 0 : _a.classList.add("has-banner"));
  }
  hideWasmBanner() {
    var _a;
  ...
  updateProgress(e) {
    if (this.label && this.isSolving) {
      const o = this.getAttribute("zeno-i18n-verifying-label") || "Verifying";
      this.label.innerText = `${o}... ${e}%`;
    }
  }
  ...
  getContrastColor(e) {
    if (!e || !e.startsWith("#"))
      return "#000000";
    const o = parseInt(e.substr(1, 2), 16), t = parseInt(e.substr(3, 2), 16), n = parseInt(e.substr(5, 2), 16), i = (d) => {
      const l = d / 255;
      return l <= 0.03928 ? l / 12.92 : Math.pow((l + 0.055) / 1.055, 2.4);
    }, s = 0.2126 * i(o) + 0.7152 * i(t) + 0.0722 * i(n), r = 1, a = 0, c = (r + 0.05) / (s + 0.05), b = (s + 0.05) / (a + 0.05);
  ...
  async startVerification() {
    ...
    try {
      this.zeno = new p({ apiEndpoint: e, siteKey: o, forceJS: t }), this.zeno.onProgress = (i) => {
        this.updateProgress(i);
      }, this.zeno.onModeDetected = (i, s) => {
        this.isUsingWasm = i === "wasm", (!s || i === "js") && (this.showWasmBanner(), this.sublabel && (this.sublabel.innerText = this.getAttribute("zeno-i18n-js-mode-label") || "Running in compatibility mode", this.sublabel.classList.add("visible")));
      };
      const { token: n } = await this.zeno.solve();
      this.handleSuccess(n);
    } catch (n) {
      console.error("Zeno Error:", n), this.setLabel(this.getAttribute("zeno-i18n-error-label") || "Error"), (_b = this.container) == null ? void 0 : _b.classList.remove("verifying"), this.isSolving = false, this.hideWasmBanner(), this.sublabel && this.sublabel.classList.remove("visible"), this.dispatchEvent(new CustomEvent("error", { detail: { message: n.toString() } }));
    }
  }
  handleSuccess(e) {
    var _a, _b, _c;
    ...
    if (o) {
      ...
      if (t) {
        ...
        if (t.style.display = n ? "inline-block" : "none", n) {
          const i = e.getBoundingClientRect(), s = e.getAttribute("zeno-floating-position") || "bottom", r = parseInt(e.getAttribute("zeno-floating-offset") || "10");
          t.style.position = "absolute", t.style.zIndex = "9999", s === "bottom" && (t.style.top = `${i.bottom + window.scrollY + r}px`, t.style.left = `${i.left + window.scrollX}px`);
        }
      }
    }
  }
