...
// src/server/src/wasm-loader.ts
import wasmModule from "./zeno.wasm";

// src/core/pkg/zeno_core.js
var zeno_core_exports = {};
__export(zeno_core_exports, {
  WasmVdfSolver: () => WasmVdfSolver,
  default: () => zeno_core_default,
  get_memory_bytes: () => get_memory_bytes,
  initSync: () => initSync,
  serialize_cycle: () => serialize_cycle,
  solve_wasm: () => solve_wasm,
...
var WasmVdfSolver = class {
  __destroy_into_raw() {
    ...
    this.__wbg_ptr = 0;
    WasmVdfSolverFinalization.unregister(this);
    return ptr;
  }
  free() {
    const ptr = this.__destroy_into_raw();
    wasm.__wbg_wasmvdfsolver_free(ptr, 0);
  }
  /**
   ...
   */
  constructor(seed_hex, discriminant_hex, vdf_iters, graph_bits) {
    const ptr0 = passStringToWasm0(seed_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(discriminant_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.wasmvdfsolver_new(ptr0, len0, ptr1, len1, vdf_iters, graph_bits);
    if (ret[2]) {
      throw takeFromExternrefTable0(ret[1]);
    }
    this.__wbg_ptr = ret[0] >>> 0;
    WasmVdfSolverFinalization.register(this, this.__wbg_ptr, this);
    return this;
  }
  /**
   ...
   */
  step(iterations) {
    const ret = wasm.wasmvdfsolver_step(this.__wbg_ptr, iterations);
    if (ret[2]) {
      throw takeFromExternrefTable0(ret[1]);
    }
...
if (Symbol.dispose) WasmVdfSolver.prototype[Symbol.dispose] = WasmVdfSolver.prototype.free;
function get_memory_bytes() {
  const ret = wasm.get_memory_bytes();
  return ret >>> 0;
}
function serialize_cycle(indices) {
  const ptr0 = passArray32ToWasm0(indices, wasm.__wbindgen_malloc);
  const len0 = WASM_VECTOR_LEN;
  const ret = wasm.serialize_cycle(ptr0, len0);
  var v2 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
  wasm.__wbindgen_free(ret[0], ret[1] * 1, 1);
  return v2;
}
function solve_wasm(seed_hex, discriminant_hex, vdf, graph_bits) {
  const ptr0 = passStringToWasm0(seed_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
  const len0 = WASM_VECTOR_LEN;
  const ptr1 = passStringToWasm0(discriminant_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
  const len1 = WASM_VECTOR_LEN;
  const ret = wasm.solve_wasm(ptr0, len0, ptr1, len1, vdf, graph_bits);
  if (ret[2]) {
    throw takeFromExternrefTable0(ret[1]);
  }
  return takeFromExternrefTable0(ret[0]);
}
function verify_proof(seed_hex, cycle, y_hex, pi_hex, discriminant_bytes, vdf, graph_bits) {
  const ptr0 = passStringToWasm0(seed_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
  const len0 = WASM_VECTOR_LEN;
  const ptr1 = passArray32ToWasm0(cycle, wasm.__wbindgen_malloc);
  const len1 = WASM_VECTOR_LEN;
  const ptr2 = passStringToWasm0(y_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
  const len2 = WASM_VECTOR_LEN;
  const ptr3 = passStringToWasm0(pi_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
  const len3 = WASM_VECTOR_LEN;
  const ptr4 = passArray8ToWasm0(discriminant_bytes, wasm.__wbindgen_malloc);
  const len4 = WASM_VECTOR_LEN;
  const ret = wasm.verify_proof(ptr0, len0, ptr1, len1, ptr2, len2, ptr3, len3, ptr4, len4, vdf, graph_bits);
  if (ret[2]) {
    throw takeFromExternrefTable0(ret[1]);
  }
  return ret[0] !== 0;
...
var zeno_core_default = __wbg_init;

// src/server/src/wasm-loader.ts
async function loadWasm() {
  await zeno_core_default(wasmModule);
  return zeno_core_exports;
}

// src/server/node_modules/zod/v3/external.js
...
// src/server/node_modules/zod/v3/ZodError.js
...
// src/server/node_modules/zod/v3/locales/en.js
...
// src/server/node_modules/zod/v3/errors.js
...
// src/server/node_modules/zod/v3/helpers/parseUtil.js
...
// src/server/node_modules/zod/v3/types.js
...
var DEFAULT_CONFIG = {
  ...
  rate_limit_ip_hour: 1e3,
  rate_limit_ip_day: 5e3
};
function getConfig(siteKey, env) {
  const getVar = (key, defaultVal, type) => {
    ...
    if (type === "regex") return new RegExp(val);
    return val;
  };
  return {
    allowed_origins: getVar("allowed_origins", null, "regex"),
    allowed_referers: getVar("allowed_referers", null, "regex"),
    vdf: Math.min(getVar("vdf", DEFAULT_CONFIG.vdf, "number"), 1e6),
    // Clamp: 1M
    graph_bits: Math.min(getVar("graph_bits", DEFAULT_CONFIG.graph_bits, "number"), 23),
    // Clamp: 23 (~8M nodes)
...
var ChallengeRequestSchema = external_exports.object({
  site_key: external_exports.string().uuid()
});
var SolutionSchema = external_exports.object({
  cycle: external_exports.any(),
  y: external_exports.any(),
  pi: external_exports.any()
});
var RedeemRequestSchema = external_exports.object({
  site_key: external_exports.string().uuid(),
  challenge_id: external_exports.string().uuid(),
  solution: external_exports.union([SolutionSchema, external_exports.string()])
});
var VerifyRequestSchema = external_exports.object({
  site_key: external_exports.string().uuid(),
  token: external_exports.string().uuid(),
  single: external_exports.boolean().optional()
});
var DeleteRequestSchema = external_exports.object({
  site_key: external_exports.string().uuid(),
  token: external_exports.string().uuid()
});
var corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  // Dynamic check performed in code
...
async function validateContext(req, siteKey, env) {
...
async function handleChallenge(req, env, ctx) {
  ...
  if (bannedObj) {
    const banData = await bannedObj.json();
  ...
  const now = Date.now();
  const expiresAt = now + config.challenge_ttl * 1e3;
  const challenge = {
    site_key,
    seed,
...
async function checkAndRateLimit(ip, config, env) {
  const NOW = Date.now();
  const KEY = `cnt:${ip}`;
  let counters = {
  ...
  if (NOW - counters.min.start > 6e4) counters.min = { start: NOW, count: 0 };
  if (NOW - counters.hour.start > 36e5) counters.hour = { start: NOW, count: 0 };
  if (NOW - counters.day.start > 864e5) counters.day = { start: NOW, count: 0 };
  counters.min.count++;
  counters.hour.count++;
  counters.day.count++;
  let limited = false;
  let reason = "";
  if (counters.min.count > config.rate_limit_ip_min) {
    limited = true;
    reason = "Rate Limit (Min)";
  } else if (counters.hour.count > config.rate_limit_ip_hour) {
    limited = true;
    reason = "Rate Limit (Hour)";
  } else if (counters.day.count > config.rate_limit_ip_day) {
    limited = true;
...
async function handleRedeem(req, env) {
  ...
  try {
    const configBits = challenge.graph_bits;
    const seed_hex = challenge.seed;
    const isValid2 = wasm2.verify_proof(seed_hex, cycle, sol.y, sol.pi, discriminantBytes, vdf, configBits);
    if (!isValid2) {
  ...
  return success({
    token: tokenId,
    expires_at: issuedAt + config.token_ttl * 1e3
  });
}
async function handleVerify(req, env) {
  ...
  if (!obj) return success({ valid: false });
  const stored = await obj.json();
  if (stored.site_key !== site_key) return success({ valid: false });
  const expiresAt = stored.issued_at + config.token_ttl * 1e3;
